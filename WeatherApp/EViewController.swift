...
class EViewController: UIViewController, PageDelegate {
    ...
    override func viewDidLoad() {
        ...
        Weather().downloadCurrentWeather { (weather: Dictionary<String, AnyObject>) in
            ...
            if let list: [AnyObject] = weather["list"] as? [AnyObject] {
                ...
                if let thisDayWeatherImage: Dictionary<String, AnyObject> = list[32] as? Dictionary<String, AnyObject> {
    ...
    func getDay() {
        let today = NSDate()
        let day = NSDate(timeInterval: 60 * 60 * 24 * 4, sinceDate: today)
